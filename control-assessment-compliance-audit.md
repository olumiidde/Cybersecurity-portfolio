# Controls and Compliance Audit — Botium Toys

> Fictional company scenario. This document assesses Botium Toys' current security controls and compliance posture against PCI DSS, GDPR, and SOC 2, and provides recommendations to close identified gaps.
...
| Manual monitoring/maintenance for legacy systems | ❌ No | The list of assets notes the use of legacy systems. These are monitored and maintained, but there is not a regular schedule in place, and procedures/policies related to intervention are unclear, which could place these systems at risk of a breach. |
...
| Locks (offices, storefront, warehouse) | ✅ Yes | The store's physical location, which includes the company's main offices, storefront, and warehouse of products, has sufficient locks. |
| Closed-circuit television (CCTV) surveillance | ✅ Yes | CCTV is installed/functioning at the store's physical location. |
| Fire detection/prevention (alarm, sprinkler system, etc.) | ✅ Yes | Botium Toys' physical location has a functioning fire detection and prevention system. |
